...
def denoise_echoes(sub, session, bidsDir, derivDir, cores):
    ...
    gm_mask = op.join(sub_prefix, 'anat', '{}_label-GM_probseg.nii.gz'.format(sub))
    wm_mask = op.join(sub_prefix, 'anat', '{}_label-WM_probseg.nii.gz'.format(sub))
    
    # confirm that masks were found
    if len(gm_mask) == 0 or len(wm_mask) == 0:
        print('No brain mask found for {}'.format(sub))
    ...
    for run in prefix_list:
        ...
        # save masks
        mask_file = op.join(outDir, '{}_task-{}_space-T1w_desc-gmwmbold_mask.nii.gz'.format(sub, task))
...
def call_tedana(sub, task, EchoFiles, MaskFile, EchoTimes, outDir):
    print(op.join(outDir, '{}_task-{}_space-T1w_tedana_report.html'.format(sub, task)))
    if os.path.isfile(op.join(outDir, '{}_task-{}_space-T1w_tedana_report.html'.format(sub, task))): # if subject tedana report already exists
        ...
        workflows.tedana_workflow(EchoFiles, 
                                  ...
                                  mask = MaskFile,
                                  out_dir = outDir,
                                  prefix = '{}_task-{}_space-T1w'.format(sub, task),
                                  fittype = 'curvefit',
                                  tedpca = 'kic',
                                  overwrite = True,
                                  gscontrol = None) 
